...
- [x] T012 [P] Validate path-param as integer (FastAPI handles 422): N/A to implement custom validator because the Project model uses integer id and FastAPI performs path-param typing/validation automatically.

Phase 3 — US1: Get project by id (P1)

- [x] T100 [US1] Implement Project repository method: get_by_id(project_id: int) -> Project | None (core/catalog/repository.py)
- [x] T101 [US1] Implement service method: get_project_by_id(project_id: int, user: Optional[User]) -> ProjectDTO (core/catalog/service.py)
